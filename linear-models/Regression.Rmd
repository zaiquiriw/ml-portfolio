...
Found on Kaggle. 
Our goal is to see if we can see how Wind Speed and Humidity relate to the difference between Apparent Temperature and Temperature.
...
By and large, we have such a large amount of data, it's difficult to notice quick correlations, aside from wind speed and temperature when it is snowing.
...
Looking at the data based on Summary does not help us much, but we can notice that the cloud of data that does not seem to have much trend is in the violet(18) and slateblue (20), or rather Mostly Cloudy and Partly Cloudy.
...
Let's change this experimental section to remove that data as well.
```{r}
diffOnly[,2:2][diffOnly$Summary==diffOnly$Summary[c(18)]] <- NA
diffOnly[,2:2][diffOnly$Summary==diffOnly$Summary[c(20)]] <- NA
diffOnly <- na.omit(diffOnly)
summary(df$Summary)
plot(diffOnly$Temperature.Diff,diffOnly$Wind.Speed..km.h.,pch=21,bg=c("yellow","green","blue")[as.integer(df$Precip.Type)])
plot(diffOnly$Temperature.Diff,diffOnly$Humidity,pch=21,bg=c("yellow","green","blue")[as.integer(df$Precip.Type)])
...
Let's start with a linear regression model with one predictor, Humidity,and summarize it. 
```{r}
simplelinreg <- lm(df$Temperature.Diff~df$Humidity,data=train)
...
```{r}
multlinreg <- lm(df$Temperature.Diff~df$Humidity+df$Wind.Speed..km.h.+df$Precip.Type,data=train)
summary(multlinreg)
```
...
```{r}
combolinreg <- lm(df$Temperature.Diff~poly(df$Humidity+df$Wind.Speed..km.h.)+df$Precip.Type,data=train)
summary(combolinreg)
```
