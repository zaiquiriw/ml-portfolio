...
cm1 <- caret::confusionMatrix(as.factor(pred), reference=test$IncomeClass)
cm1
```
...
```{r}
library(ROCR)
p2 <- predict(nb1, newdata=test, type="raw")

prf <- performance(p2, measure = "tpr", x.measure = "fpr")
plot(prf)
# Compute AUC
auc <- performance(pr, measure = "auc")
auc <- auc@y.values[[1]]d
auc
```
...
```{r}
# Logistic Regression
t <- table(pred, test$IncomeClass)
tp <- t[1]
fp <- t[3]
fn <- t[2]
tn <- t[4]
mltools::mcc(as.factor(pred), test$IncomeClass)
```
